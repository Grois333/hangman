...
//const alph = 'abcdefghijklmnopqrstuv'.split('');
...
const render = () => {
  ...
  for (let i = 0; i < state.puzzle.length; i++) {
    ...
    newPuzzleHTML += `<span class="${state.puzzle[i] === ' '? 'space' : ''}">` + letterToRender + '</span>';
  ...
  for (let i = 0; i < alphabet.length; i++) {
    if ((state.goodLetters.indexOf(alphabet[i]) === -1) && (state.badLetters.indexOf(alphabet[i]) === -1)) {
